...
for i=1:3
    ...
    if S.SE(i).sourceP == 60 
        ...
        S.IE(nie+counterie).u(1) = S.SE(i).u(1)+(S.SE(i).v(1)-S.SE(i).u(1))/3;
        S.IE(nie+counterie).u(2) = S.SE(i).u(2)+(S.SE(i).v(2)-S.SE(i).u(2))/3;
        S.IE(nie+counterie).v(1) = S.SE(i).v(1);
        ...
        S.SE(i).v(1) = S.SE(i).u(1)+(S.SE(i).v(1)-S.SE(i).u(1))/3; %new distal coordinates for source element
    ...
    elseif S.SE(i).sourceP == 40
        ...
        S.IE(nie+counterie).v(1) = S.SE(i).u(1)+(S.SE(i).v(1)-S.SE(i).u(1))/3;
        S.IE(nie+counterie).v(2) = S.SE(i).u(2)+(S.SE(i).v(2)-S.SE(i).u(2))/3;
        S.IE(nie+counterie).type = S.SE(i).type;
        S.IE(nie+counterie).length = lengte(S.IE(nie+counterie).u,S.IE(nie+counterie).v);
        S.IE(nie+counterie).radius = S.SE(i).radius;
        parent = nie+counterie; %remember this element for simulation of random structure in other perfusion area of this element
        
        S.SE(i).u(1) = S.SE(i).u(1)+(S.SE(i).v(1)-S.SE(i).u(1))/3;
        S.SE(i).u(2) = S.SE(i).u(2)+(S.SE(i).v(2)-S.SE(i).u(2))/3;
        S.SE(i).node = S.nin+1;
        S.SE(i).length = lengte(S.SE(i).u,S.SE(i).v);
    ...
    segment(1).v(1) = 0+x*dx; %distal coordinates new segment
    segment(1).v(2) = 0+y*dy; %distal coordinates new segment
    ...
    segment(1).v(1) = 0+x*dx;
    segment(1).v(2) = 0+y*dy;
    segment(1).length = lengte(segment(1).u,segment(1).v);
    segment(1).node = S.nin+1;
     ...
     for k=2:Nterm
         ...
         for mp=1:10
             ...
             for Ntoss=1:25
                 x=round(rand*nx);
                 y=round(rand*ny);
                 %ed: it looks like random positions are taken until
                 %finally a position is found that belongs to the perfusion
                 %area of the vessel of interest. This seems not handy and
                 %if there are no positions available this hangs. 
                 while  x == 0 || y == 0 || S.matrix(y,x) ~= i+nse 
                     x=round(rand*nx);
                     y=round(rand*ny);
                 end
